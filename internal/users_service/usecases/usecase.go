...
type (
	...
	UsersRepository interface {
		...
		//CreateUser - создать юзера
		CreateUser(ctx context.Context, req  models.SignUpUserEmailRequest,hash string,provider string) (*dto.CreateUserResponse, error)
		//UpdateProfile - обновить профиль (никнейм - уникальный, информация о себе, аватарка)
		UpdateProfile(ctx context.Context, req models.UpdateProfileRequest) (*models.UpdateProfileResponse, error)
		//Получить юзера по никнейму
		GetUser(ctx context.Context, req models.SearchByNicknameRequest) (*dto.UsersRepo, error)
	}
)
